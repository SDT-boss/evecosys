 ...
 *
 * The production LeftRailShell (components/layout/LeftRailShell.tsx) requires
 ...
 *
 * When modifying LeftRailShell for real use, ensure any structural changes
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ borderBottom: "1px solid var(--ds-color-neutral-grey-20)" }}
      >
        <div
          className="w-7 h-7 rounded-[var(--ds-radius-sm)] flex items-center justify-center text-xs font-bold"
          style={{ background: "var(--ds-color-brand-primary)", color: "white" }}
        >
          E
        </div>
        <span className="font-semibold text-sm tracking-wide" style={{ color: "var(--ds-color-neutral-ink)" }}>EVEcosys</span>
      </div>
        ...
        {NAV_ITEMS.map(({ label, icon: Icon, active: defaultActive, badge }) => {
          ...
          return (
            <a
              ...
              style={{
                background: isActive ? "rgba(0,134,132,0.08)" : "transparent",
                color: isActive
      ...
      <div
        ...
        style={{ borderTop: "1px solid var(--ds-color-neutral-grey-20)" }}
      >
        <div
        ...
        <div className="flex-1 min-w-0">
          <p className="text-xs font-semibold truncate" style={{ color: "var(--ds-color-neutral-ink)" }}>Jane Doe</p>
          <p className="text-xs truncate" style={{ color: "var(--ds-color-neutral-grey-60)" }}>Fleet Manager</p>
        </div>
      </div>
    </aside>
  );
...
function MockContent() {
  return (
    <main className="flex-1 overflow-auto p-6 flex flex-col gap-6" style={{ background: "#f2f5f2" }}>
...
const meta: Meta = {
  title: "Compositions/LeftRailShell",
  tags: ["autodocs"],
  parameters: {
    layout: "fullscreen",
    docs: {
      description: {
        component:
          "Static mock of the Manager/Admin LeftRailShell layout. The production shell (`components/layout/LeftRailShell.tsx`) requires live Supabase auth and Next.js routing. This story demonstrates the visual structure, sidebar nav tokens, content utility bar, and content grid — all driven by `--ds-*` tokens.",
      },
    },
  },
};
...
export const SidebarNav: Story = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        story:
          "The left rail sidebar in isolation. Active state uses a 3px Jade left border as the primary selection indicator — colour is never the sole signal (WCAG 1.4.1).",
      },
    },
  },
  render: () => (
